...
(function () {
    ...
    // unsafe: paint flashing
    DevTools.unsafe.register("debugger", ["paintFlash"]);
    ...
    DevTools.registerPlugin("debugger", {
        ...
        onLoad(api) {
            ...
            api.commands.register("debugger.paintFlash", () => {
                if (!api.ensureUnsafe("debugger.paintFlash")) return;
        ...
        startMutations() {
            ...
            this.mutationObserver = new MutationObserver((muts) => {
                ...
                for (const m of muts) {
                    ...
                    if (root && root.host && root.host.id === "bjorn-devtools-host") continue;
        ...
        startLagMeter() {
            ...
            this.lagInterval = setInterval(() => {
                ...
                last = now;
                if (drift > 12) api.log(`Lag: ${drift.toFixed(1)}ms`);
            }, 100);
        ...
        onMount(view) {
            view.innerHTML = `
                ...
                <ul style="font-size:11px;line-height:1.4;">
                    <li>spy.clicks / spy.keys / spy.scroll / spy.stop</li>
                    <li>mutations.on / mutations.off</li>
                    <li>fps / fps.stop</li>
                    <li>lag / lag.stop</li>
                    <li>console.spy</li>
                    <li>debugger.paintFlash / paint.stop</li>
                </ul>`;
        }
    });
})();
